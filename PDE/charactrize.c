...
#define D 1.0//(1.25*pow(10,-4))//Diffusion Coefficient
#define N 32
...
double *out_force_charcterize(mesh_t *mesh,double *u_old){
    ...
    for(int l=1;l<=ne;l++){
        ...
        /*===================上流点の要素ないにおける積分結果======================*/
        //l番目の要素の上流点を探す
        double *KL_G=g_Kl(mesh,l);//lの重心の座標
        double *u_Kl=u(KL_G[1],KL_G[2]);//流速u
        int upstream=search_past_point(mesh,KL_G[1],KL_G[2],u_Kl,delta_t);//Kl_Gの上流点が含まれる要素番号，uに依存した上流要素
        // int upstream=count(mesh,KL_G[1]+delta_t*KL_G[2],KL_G[2]-delta_t*KL_G[1]);
        ...
        for(int i=1;i<=n;i++){  
            for(int j=1;j<=n;j++){
                // int ver1=elnp[l][i],ver2=elnp[l][j];//lを構成するi番目の節点番号
                /*==========∫φiφjdxの計算結果代入==============*/
                double I_up=Int(mesh,i,j,upstream);//i->Pi,j->Pjとして考えている．
                /*==========================================*/
                M_up[i][j]+=I_up;
            }
        }
        ...
        /*==================要素質量外力ベクトル======================*/
        for(int i=1;i<=n;i++){
            u_old_vector[i]=u_old[elnp[upstream][i]];//上流点用の値格納，RHS(u_old,ver1,x,y);
        }
        ...
        free_dvector(KL_G,1,dim);
        free_dvector(u_Kl,1,dim);
    }
